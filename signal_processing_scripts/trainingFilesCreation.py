...
DataCounter = 0 #501
StartingNumber = 0 #680
SamplesToRecord = 20 #20, change this depending on how many samples you want to record (each sample is a txt file of 250 points)
...
FileNumber = [StartingNumber, StartingNumber]
FileTypes = ["Up", "Down"]
CurrFileType = 0
LastFileType = 0
GeneratePlots = False
...
def print_raw(var):
    ...
    if(DataCounter < 250): #250 samples = 1 second (250Hz)
        ...
        if(DataCounter > 500): # First run, ignore it
            DataCounter = 0
            CurrFileType = random.randint(0,1)  # random.randint(0,7)
...
# board = OpenBCICyton(port='/dev/tty.usbserial-DM01N7JO', daisy=True) # Check this!!
board = OpenBCICyton(port='/dev/ttyUSB*', daisy=True) # Replace star with COM port
